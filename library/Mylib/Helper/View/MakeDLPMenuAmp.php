 ...
 */
class Mylib_Helper_View_MakeDLPMenuAmp extends Zend_View_Helper_Abstract
{
     ...
     */
    public function makeDLPMenuAmp($dlpMenus, $vendorFullUrl,$dlpMenusData,$tabUrl = null)
    {
        if($tabUrl!= ''){
            $tabMenuType = $tabUrl;
        }else{
       ...
       foreach ($dlpMenus as $k => $menu) {
            ...
            if($hrefUrl!= ''){
                ...
                $strTab .= '<a '.$class.' data-content="'.$menu['menu_type'].'" href="'.$hrefUrl.'?amp=1">'.$menu['menu_name'].'</a>';
