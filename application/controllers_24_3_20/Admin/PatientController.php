...
class Admin_PatientController extends Mylib_Controller_AdminbaseController {
...
//dd($insertedId);
                    ...
                    $this->_redirect(ADMIN_BASE_URL . 'patient/addedit');
                }
                } else {
    ...
    public function deletepatientAction() {
        ...
        if ($request->getPost()) {
            ...
            if($userID != 0){
                $data1['status'] = 0;
                $data1['updated_at'] = $nowDateTime;
                $deleteUser = $this->_userResource->updateUserDetail($data1,$userID);
            }
