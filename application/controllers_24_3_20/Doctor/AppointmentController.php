    ...
    public function deleteappointmentAction() {
     ...
     public function updatestatusAction() {
        ...
        if ($data['status'] != '') {
            ...
            $appnTData = $this->_appointmentResource->fetchAppointmentData('',$id);
            //dd($appnTData);
            if(!empty($appnTData['contact_no'])){
            $arrMno[] = '91'.$appnTData['contact_no'];
            $PatientAppntSMSTemp       = "Appointment has been fixed msg_Bidhan";                         
        ...
        if ($data = $request->getPost()) {
            ...
            if (!empty($appntId)) {
                ...
                if ($inputData->isValid()) {
                    ...
                    $cntAppId = strlen($appntId);
                    $vChatUrlRandNo = '';
                    $totNOCnt = 10;
                    if($cntAppId<$totNOCnt){
                        $vChatUrlRandNo = $appntId.$this->randomWithLength($totNOCnt-$cntAppId);
                    }  else {
                        $vChatUrlRandNo = $this->randomWithLength($totNOCnt);
                    }
                    ...
                    $appntData = array(
                                ...
                                'video_chat_link' => VIDEO_CHAT_URL.$vChatUrlRandNo
                            );
                     //dd($appntData);   
                    $updateA = $this->_appointmentResource->updateAppointmentDetail($appntData,$appntId);
