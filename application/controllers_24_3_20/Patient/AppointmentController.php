       ...
       global $appointmentType;
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            foreach ($result['result'] as $k => $val) {
                ...
                $id = $val['id'];
                $pid = $val['patient_id'];
                $apType = $val['appointment_type'];
                $selectStatus = '';
                $selectStatus .= "<select id='status_$id'>";
                ...
                if($val['appointment_type'] == 1){
                    ...
                    if(($val['video_chat_datetime'] != '') && (strtotime($nowDateTime)) >= strtotime($val['video_chat_datetime'])){
                ...
                $actionUpdate = "<a onclick='return patientEdit($id,$apType);' href='javascript:void(0);'><img width='15' height='15' src='".WEBSITE_Image."/view.png' alt='View' title='View'></a>$videoChatLink";               
//                if($val['status'] == 1){
            ...
            if ($inputData->isValid()) {
                ...
                if (!empty($pid) && !empty($appntId)) {
                    ...
                    //$this->_redirect(PATIENT_BASE_URL . 'appointment/video-consultation-addedit/patient_id/'.$pid.'/appnt_id/'.$appntId);
                                ...
                                $this->_flashMessenger->addMessage(array('success' => 'Appointment successfully sent.'));
    ...
    public function consultationNoteAction() {
       ...
       // global $status;
        ...
        if (!empty($appntId)) {
            ...
            $appntData = $this->_appointmentResource->fetchAppointmentWithDoctorInfo($appntId);
            //dd($appntData);
            $formdata = array(
                'id'=> $appntData['id'],
