...
class Laboratory_UserController extends Mylib_Controller_LaboratorybaseController {
    ...
    public function deleteAction() {
        ...
        if (isset($id)) {
            ...
            if ($this->_userResource->deleteUserData($id)) {
                $this->_redirect(LABORATORY_BASE_URL . 'user/index');
            } else {
                $this->_redirect(LABORATORY_BASE_URL . 'user/index');
            }
        }
        ...
        if ($request->isPost()) {
            ...
            if ($inputData->isValid()) {
                ...
                if ($insert) {
                    ...
                    $this->_redirect(LABORATORY_BASE_URL . 'user/welcome-msg-list');
                } else {
                ...
                $this->_helper->FlashMessenger()->setNamespace('error')->addMessage($msg);
                $this->_redirect(LABORATORY_BASE_URL . 'user/welcome-msg');
            }
        }
    }
    ...
    public function sendEmailMsgAction() {
        ...
        if ($request->isPost()) {
            ...
            if ($inputData->isValid()) {
                ...
                if ($insert) {
                    $this->_helper->FlashMessenger()->setNamespace('success')->addMessage('Message is successfully sent.');
                    $this->_redirect(LABORATORY_BASE_URL . 'user/all-sent-email-mobile-list/message-section/1');
                } else {
                ...
                $msg = $this->getValidatorErrors($inputData->getMessages());
                $this->_helper->FlashMessenger()->setNamespace('error')->addMessage($msg);
                $this->_redirect(LABORATORY_BASE_URL . 'user/msg-email');
            }
        }
    }
    ...
    public function msgDeleteAction() {
        ...
        if (!empty($data['id']) && !empty($data['message-section'])) {
            ...
            if ($this->_userResource->deleteSentEmailUserData($arrIds)) {
                $this->_redirect(LABORATORY_BASE_URL . 'user/all-sent-email-mobile-list/message-section/' . $data['message-section']);
            } else {
                $this->_redirect(LABORATORY_BASE_URL . 'user/view-msg/message-section/' . $data['message-section'] . '/id/' . $data['id']);
            }
        }
    }
    ...
    public function sendMobileMsgAction() {
        ...
        if ($request->isPost()) {
            ...
            if ($inputData->isValid()) {
                ...
                if ($insert) {
                    $this->_helper->FlashMessenger()->setNamespace('success')->addMessage('Message is successfully sent.');
                    $this->_redirect(LABORATORY_BASE_URL . 'user/all-sent-email-mobile-list/message-section/' . $params['message_section']);
                } else {
                ...
                $this->_helper->FlashMessenger()->setNamespace('error')->addMessage($msg);
                $this->_redirect(LABORATORY_BASE_URL . 'user/msg-mobile');
            }
        }
    }
