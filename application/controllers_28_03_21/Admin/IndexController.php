...
class Admin_IndexController extends Mylib_Controller_AdminbaseController
{
        ...
        if ($request->isPost()) {
          ...
          //  if($request->getParam('captchCode') == $this->captcha){
                if ($this->_process(array('username' => $request->getParam('username'), 'password' => $request->getParam('password')))) {
                    ...
                    $this->_redirect(ADMIN_BASE_URL.'dashboard/index');
                }
          //  }
            $this->_redirect(ADMIN_BASE_URL.'index/login/s/1');
        } else {
    ...
    protected function _process($values)
    {
        ...
        $objRequest->setParam('admin', true);
        $objRequest->setParam('userRoleType', ADMIN_ROLE);
