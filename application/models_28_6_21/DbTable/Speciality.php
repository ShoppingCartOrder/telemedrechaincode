 ...
 */
class Application_Model_DbTable_Speciality extends Mylib_Model_BaseModel {

    /**
     * Defining class variable for users table name.
     */
    protected $_name = 'hms_speciality';
    ...
    public function getSpecialityById($id) {
    	...
    	public function fetchAllSpeciality() {
        
       
            
            $db = $this->getAdapter();
            $query = $db->select()
		->from(array('d'=>'hms_speciality'),array('*'));
    ...
    public function updateSpecialityDetail($dbdata, $data) {
        $where['id =?'] = $data;
    ...
    public function updateSpecialityData($data) {
    ...
    public function allSpecialitys($params = null)
    {
        if (empty($params))
            return false;
