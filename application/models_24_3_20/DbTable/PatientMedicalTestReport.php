 ...
 */
class Application_Model_DbTable_PatientMedicalTestReport extends Mylib_Model_BaseModel {

    /**
     * Defining class variable for users table name.
     */
    protected $_name = 'patient_medical_test_report';
     ...
     * @return string.
     */
    public function addPatientMedicalTestReport($data) {
        try {
            //dd($data);
    ...
    public function updatePatientMedicalTestReport($dbdata, $id) {
        $where['id =?'] = $id;
        return $this->getAdapter()->update($this->_name, $dbdata, $where);
    }

    public function getPatientMedicalTestReport($id) {
        try{
        if($id){
            $db = $this->getAdapter();
            $select = $db->select()
                    ->from('patient_medical_test_report', '*')
                    ->where('id = ?', $id);
    ...
    public function allMedicalTestReportDetails($params = null)
    {
        if (empty($params))
            return false;
        ...
        $select = $this->getAdapter()->select()
                ->from(array('pd' => $this->_name), $fields);

        $selectCount = $this->getAdapter()->select()
                ->from(array('pd' => $this->_name), array(new Zend_Db_Expr("count(pd.id)  AS count")));
        ...
        if(isset($params['userRoleType']) && ($params['userRoleType'] == DOCTOR_ROLE)){
            $genCond = " AND pd.patient_id = ".$params['condition']['patient_id'];
            
        }else{
           $genCond = " AND pd.patient_id = ".$this->modelPatientId; 
        }
        
        
        if (!empty($params['condition']['test_name'])) {
            $where .= " AND pd.test_name LIKE '%".$params['condition']['test_name']."%'";
        }
        if (!empty($params['condition']['test_result'])) {
            $where .= " AND pd.test_result LIKE '%".$params['condition']['test_result']."%'";
        }
        if (!empty($params['condition']['note'])) {
            $where .= " AND pd.note LIKE '%".$params['condition']['note']."%'";
        }
        if (!empty($params['condition']['prescribed_by'])) {
            $where .= " AND pd.prescribed_by LIKE '%".$params['condition']['prescribed_by']."%'";
        }
        if (!empty($params['condition']['test_center'])) {
            $where .= " AND pd.test_center LIKE '%".$params['condition']['test_center']."%'";
        }
       
        if (!empty($params['condition']['test_date'])) {
        	$where .= " AND date(pd.test_date) = '" . date(DB_DATE_FORMAT,strtotime($params['condition']['test_date'])) . "'";
        }      
        
        if (!empty($params['condition']['created_at'])) {
        	$where .= " AND date(pd.created_at) = '" . date(DB_DATE_FORMAT,strtotime($params['condition']['created_at'])) . "'";
        }
