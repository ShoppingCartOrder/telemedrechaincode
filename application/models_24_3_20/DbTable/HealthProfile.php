 ...
 */
class Application_Model_DbTable_HealthProfile extends Mylib_Model_BaseModel {

    /**
     * Defining class variable for users table name.
     */
    protected $_name = 'patient_health_profiles';
     ...
     * @return string.
     */
    public function addHealthProfile($data) {
        try {
    ...
    public function getHealthProfileDetails($id) {
        try{
        if($id){
            $db = $this->getAdapter();
            $select = $db->select()
