 ...
 */
class Application_Model_DbTable_PatientPrescriptionDetails extends Mylib_Model_BaseModel {

    /**
     * Defining class variable for users table name.
     */
    protected $_name = 'patient_prescription_details';
     ...
     * @return string.
     */
    public function addpatientPrescriptionDetails($data) {
        try {
    ...
    public function updatePatientPrescriptionDetails($dbdata, $id) {
        $where['id =?'] = $id;
        return $this->getAdapter()->update($this->_name, $dbdata, $where);
    }

    public function getPatientPrescriptionDetails($id) {
        try{
        if($id){
            $db = $this->getAdapter();
            $select = $db->select()
                    ->from('patient_prescription_details', '*')
                    ->where('id = ?', $id);
    ...
    public function updateAppointmentData($data) {
     ...
     public function allPrescriptionDetails($params = null)
    {
        if (empty($params))
            return false;
        ...
        if (!empty($params['condition']['medicine_name'])) {
            $where .= " AND pd.medicine_name LIKE '%".$params['condition']['medicine_name']."%'";
        }
        if (!empty($params['condition']['dosage'])) {
            $where .= " AND pd.dosage LIKE '%".$params['condition']['dosage']."%'";
        }
        if (!empty($params['condition']['duration'])) {
            $where .= " AND pd.duration LIKE '%".$params['condition']['duration']."%'";
        }
        if (!empty($params['condition']['side_effects'])) {
            $where .= " AND pd.side_effects LIKE '%".$params['condition']['side_effects']."%'";
        }
