 ...
 */
class Application_Model_DbTable_ImmunizationRequest extends Mylib_Model_BaseModel {

    /**
     * Defining class variable for users table name.
     */
    protected $_name = 'hms_immunization_request';

    public function addImmunizationRequest($data) {
        try {
    ...
    public function deleteImmunizationRequest($condition) {
     ...
     * @return array.
     */
    public function allImmunizationRequest($params = null) {
...
//echo $selectCount; die;
    ...
    public function updateImmunizationRequestStatus($dbdata, $data) {
        $where['id =?'] = $data;
        return $this->getAdapter()->update($this->_name, $dbdata, $where);
    }

    public function fetchImmunizationRequestByReqId($reqid) {
        ...
        if (!empty($reqid)) {
            ...
            $query = $db->select()
                    ...
                    ->joinInner(array('iv' => 'hms_immunization_vaccine_details'), 'iv.id = lr.vaccine_id', array('iv.age'))
                    ->where('lr.id = ?', $reqid);
            //echo $query; die;
