 ...
 */
class Application_Model_DbTable_Investigation extends Mylib_Model_BaseModel {

    /**
     * Defining class variable for users table name.
     */
    protected $_name = 'hms_investigation';
...
public function addInvestigation($data) {
    ...
    public function getInvestigationByRegistrationNo($pid, $rid) {
        try {
            if ($pid && $rid) {
                $db = $this->getAdapter();
                $select = $db->select()
    ...
    public function updateInvestigationDetail($dbdata, $id) {
        $where['id =?'] = $id;
    ...
    public function deleteInvestigationData($id) {

        $where = $this->getadapter()->quoteInto('id = ?', $id);
        return $this->delete($where);
    }

    public function updateInvestigationData($data) {
    ...
    public function allInvestigations($params = null) {
        if (empty($params))
            return false;
