 ...
 */
class Application_Model_DbTable_Prescribe extends Mylib_Model_BaseModel {

    /**
     * Defining class variable for users table name.
     */
    protected $_name = 'hms_doctor_prescribe';
     ...
     * @return string.
     */
    public function addPrescribe($data) {
        try {
    ...
    public function getPrescriptionByRegistrationNo($pid, $rid) {
        try {
            if ($pid && $rid) {
                $db = $this->getAdapter();
                $select = $db->select()
