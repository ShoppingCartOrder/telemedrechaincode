    ...
    public function registerAction() {
        ...
        if ($data = $request->getPost()) {
            ...
            if ($inputData->isValid()) {
                ...
                //dd($email);
                $getUser = $this->_userResource->checkuserByMobile($params['mobile_no']);
//dd($getUser);
                if ($getUser) {

                    $this->_helper->FlashMessenger()->setNamespace('error')->addMessage('Mobile no already exist.');
                } else {
...
//dd($userdata);
                    ...
                    if ($uinsertedId) {
                        ...
                        if(!empty($params['doa']) && !empty($params['doctor'])){
                            ...
                            if (isset($params['room_no']) && !empty($params['room_no'])) {
                                $patientApntData['room_no'] = $params['room_no'];
                            }
                            if (isset($params['bed_no']) && !empty($params['bed_no'])) {
...
//dd($msg);
   ...
   public function patientchargesAction() {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            $result = $this->_appointmentsResource->allAppointments($params);
            //dd($result);
            $responce = new stdClass();
            $responce->page = $result['page'];
            $responce->total = $result['total'];
            ...
            foreach ($result['result'] as $k => $val) {
                ...
                $responce->rows[$k]['id'] = $id;
                $actionUpdate = '';
                $appntChrgLstUrl = HOSPITAL_BASE_URL.'patient/patient-charges-details/appntid/'.$val['id'];
...
//dd($patientFormData);
    ...
    public function patientChargesDetailsAction() {
        global $nowDateTime;
