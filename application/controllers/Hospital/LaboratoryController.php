    ...
    public function indexAction() {
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            foreach ($result['result'] as $k => $val) {
                ...
                $labResultURL = HOSPITAL_BASE_URL . 'laboratory/lab-test-result-addedit/reqid/' . $id;

                if($val['status'] == 1){
                  $printReportUrl = HOSPITAL_BASE_URL . 'laboratory/printlabtestreport/reqid/' . $id;  
                ...
                if (!empty($params['result'])) {
                    ...
                    foreach ($params['result'] as $key => $result) {
                        ...
                        if ($result) {
                            ...
                            $totalResults[] = $key;
                            $totalResults[] = $result;
                            $totalResults[] = $this->hospitaluserId;
                            $rsltCnt++;
                        }
                    }
                }
...
//dd($insertedId);
                if ($insertedId) {
                    ...
                    $this->_helper->FlashMessenger()->setNamespace('error')->addMessage('Lab Result data is not inserted.Please try again.');
                    $this->_redirect(HOSPITAL_BASE_URL . 'laboratory/lab-test-result-addedit/reqid/' . $reqid);
                }
            }
        }
    }
